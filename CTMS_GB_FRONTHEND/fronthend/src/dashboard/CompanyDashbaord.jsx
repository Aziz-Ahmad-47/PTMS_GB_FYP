...
const CompanyDashboard = () => {
  ...
  const { seatOffers, hireOffers, activeOffers, revenueStats } = useMemo(() => {
    const seatOffers = transports.filter(t => t.offer_type === 'offer_sets');
    const hireOffers = transports.filter(t => t.offer_type === 'whole_hire');
    const activeOffers = transports.filter(t => t.is_active !== false);
    
    const totalRevenue = transports.reduce((sum, transport) => {
