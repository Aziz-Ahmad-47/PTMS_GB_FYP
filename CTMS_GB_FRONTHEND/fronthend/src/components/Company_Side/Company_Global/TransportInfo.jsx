...
const sendStatusUpdateEmail = async (booking, statusUpdate, token) => {
  try {
    ...
    //   `${API_BASE_URL}/notifications/send-status-email/`,
      ...
      `http://localhost:8000/api/auth/notifications/send-status-email/`,
      `http://127.0.0.1:8000/api/notifications/send-status-email/`
    ];
      ...
      const processedTickets = ticketsData.map(ticket => {
        ...
        if (matchingManual?.screenshot_url) {
          ...
          if (screenshot.startsWith('http') || screenshot.startsWith('data:image')) {
            ...
            screenshotUrl = `http://localhost:8000${screenshot}`;
          } else {
            screenshotUrl = `http://localhost:8000/media/${screenshot}`;
          }
        }
