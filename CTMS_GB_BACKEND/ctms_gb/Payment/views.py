...
# ////////////////////////////
# vehicle_booking/views.py

# views.py mein add karein
...
@api_view(['GET'])
def get_full_vehicle_timings(request, vehicle_id):
    """
    Sirf full vehicle bookings ke timings return karta hai
    """
    try:
        print(f"Fetching full vehicle timings for vehicle_id: {vehicle_id}")
        bookings = Booking.objects.filter(
            vehicle_id=vehicle_id,
            is_full_vehicle=True
        ).exclude(
            booking_status__in=['CANCELLED', 'EXPIRED', 'FAILED']
        ).order_by('arrival_date', 'arrival_time')
        
        print(f"Found {bookings.count()} bookings for vehicle {vehicle_id}")
        ...
        for booking in bookings:
            ...
            if booking.arrival_date:
                arrival_date_str = booking.arrival_date.isoformat() if hasattr(booking.arrival_date, 'isoformat') else str(booking.arrival_date)
            
            arrival_time_str = None
            if booking.arrival_time:
                arrival_time_str = booking.arrival_time.isoformat() if hasattr(booking.arrival_time, 'isoformat') else str(booking.arrival_time)
            
            hold_expires_str = None
            if booking.hold_expires_at:
                hold_expires_str = booking.hold_expires_at.isoformat() if hasattr(booking.hold_expires_at, 'isoformat') else str(booking.hold_expires_at)
            
            formatted_timings.append({
                'arrival_date': arrival_date_str,
                'arrival_time': arrival_time_str,
                'hold_expires_at': hold_expires_str,
        ...
        return Response({
            ...
            'count': len(formatted_timings),
            'message': f'Found {len(formatted_timings)} full vehicle bookings'
        })
        
    except Exception as e:
        print(f"Error in get_full_vehicle_timings: {str(e)}")
        import traceback
        traceback.print_exc()
